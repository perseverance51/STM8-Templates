...
//////////
static uint8_t setAD5280(uint8_t addr, uint8_t res) {
  
  uint8_t  err, buf[2] = {0x00, 0x00};
...
void loop() {
  ...
  uint8_t  result, buf[2] = {0x00, 0x00};  // err state and I2C buffer
  
  // set new resistance
  buf[1] = res;
  result = setAD5280(ADDR_POTI, res);
  res += 8;

  // print to terminal
  printf("%d Ohm, err=%d\n", (int) (((long) res * 20000L) >> 8), (int) result);
